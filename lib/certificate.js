...
export async function downloadCertificate(userName, completionDate) {
  ...
  // Fetch logo as base64 so it renders in the print window without network dependency
  var logoDataUri = '';
  try {
    var logoRes = await fetch(window.location.origin + '/images/adg-logo.png');
    var blob = await logoRes.blob();
    logoDataUri = await new Promise(function(resolve) {
  ...
  // Logo — embedded as base64 so it prints reliably
