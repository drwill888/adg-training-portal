...
export default function ModuleTemplate({ config }) {
  ...
  // ─── Purpose Threads collapsible (Calling module commitment step) ─────────
    ...
    async function triggerReport() {
      try {
        ...
        fetch('/api/mid-journey/trigger-report', {
          ...
          body: JSON.stringify({ userId: ss.user.id, email: ss.user.email, moduleId: moduleNum, stepNumber: step }),
  ...
  var handleCertificate = async function() {
    ...
    try {
      if (user?.user_metadata?.full_name) {
        ...
        if (profileRes.data?.full_name) certName = profileRes.data.full_name;
      }
    } catch (e) {}
    if (!certName && email) {
      ...
      case "commitment":
        return (
          <div className="space-y-6">
            ...
            {/* ─── Purpose Threads — Calling module only, collapsed by default ── */}
            {moduleNum === 1 && (
              <div className="rounded-xl overflow-hidden" style={{ background: "rgba(10,45,82,0.4)", borderLeft: "4px solid " + GOLD, border: "1px solid rgba(253,210,13,0.08)" }}>
                <button
                  onClick={function() { setPurposeThreadsOpen(function(o) { return !o; }); }}
