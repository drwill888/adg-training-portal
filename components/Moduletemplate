...
// Each module page passes a config object with all module-specific data
  ...
  return (
    <div style={{ display: "flex", gap: 4, alignItems: "center", flexShrink: 0 }}>
      {[1, 2, 3, 4, 5].map(score => (
        <button
          ...
          style={{
            ...
            fontSize: 13, fontWeight: "bold", border: "none", cursor: "pointer",
      ...
      <button
        ...
        style={{
          height: 32, padding: "0 7px", borderRadius: 6,
...
function DiagnosticSection({ diagnostic, scores, setScores, accent, label }) {
  return (
    <div className="space-y-4">
      <div className="mb-2">
        <p className="text-xs uppercase tracking-widest font-semibold mb-1" style={{ color: accent }}>
          {label}
        </p>
        <p className="text-sm" style={{ color: "#666" }}>
          Rate each statement: 1 = Not true · 5 = Absolutely true · N/A = Does not apply
        </p>
      </div>
      {diagnostic.map(d => (
...
function SectionHead({ children, sub }) {
  return (
    <div className="mb-5">
      <h3 className="text-xl sm:text-2xl font-bold" style={{ color: NAVY, fontFamily: "'Cormorant Garamond', serif" }}>
        {children}
      </h3>
...
function downloadBlueprint(title, commitments, summary) {
  const html = `<html><head><meta charset='utf-8'/>
    <style>body{font-family:Georgia,serif;color:#333;line-height:1.7;margin:40px;}
    h1{color:#021A35;font-size:26px;}h2{color:#C8A951;font-size:18px;border-bottom:2px solid #FDD20D;padding-bottom:4px;margin-top:28px;}
    .section{background:#FFF9E6;border-left:4px solid #C8A951;padding:16px;margin:20px 0;border-radius:4px;}
    p{margin:8px 0;}</style></head>
    <body>
    <h1>🎯 ${title} Blueprint</h1>
    <p><strong>5C Leadership Blueprint · Awakening Destiny Global</strong></p>
    <p style="color:#888;font-style:italic;">Generated ${new Date().toLocaleDateString()}</p>
    ${summary ? `<div class="section"><h2>AI Analysis</h2><p>${summary.replace(/\n/g, "<br/>")}</p></div>` : ""}
    <h2>Your Commitments</h2>
    ${Object.entries(commitments).map(([k,v]) => `<p><strong>${k.replace(/_/g," ")}:</strong> ${v||"(not completed)"}</p>`).join("")}
    <footer style="margin-top:48px;padding-top:16px;border-top:1px solid #ddd;font-size:11px;color:#aaa;">
    <p>© Awakening Destiny Global · awakeningdestiny.global</p>
    <p>Review this blueprint regularly. Let it anchor your decisions and deepen your alignment.</p>
    </footer></body></html>`;
  const blob = new Blob([html], { type: "application/msword" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a"); a.href = url;
  a.download = `${title}-Blueprint.doc`; a.click();
...
export default function ModuleTemplate({ config }) {
  ...
  const generateSummary = async () => {
    setLoading(true);
    try {
      const commitStr = Object.entries(commitments)
        .map(([k, v]) => `${k.replace(/_/g, " ")}: ${v || "(not provided)"}`)
        .join("\n");
  ...
  const renderStep = () => {
    switch (cur.id) {
      ...
      case "activation":
        return (
          <div className="space-y-6">
            <SectionHead sub="Set aside distractions. You are entering formational territory.">
              Welcome to {title}
            </SectionHead>
            <div className="p-6 rounded-2xl" style={{ background: NAVY, color: "#fff" }}>
              {question && (
                <>
                  <p className="text-xs uppercase tracking-widest mb-2 font-semibold" style={{ color: accent }}>
                    The Central Question
                  </p>
                  <p className="text-lg italic mb-4" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
                    "{question}"
                  </p>
                </>
              )}
            ...
            <div>
              <SectionHead sub="Take three minutes in silence. Write the first honest answer that surfaces.">
                Activation Prompts
              </SectionHead>
      ...
      case "pre-diagnostic":
        return (
          <DiagnosticSection
            ...
            scores={preScores}
            setScores={setPreScores}
            accent={accent}
            label="Pre-Teaching Self-Assessment"
          />
        );
      ...
      case "teaching":
        return (
          <div className="space-y-8">
            {contrastTable && (
              <div className="mb-2">
                <SectionHead sub="Which column describes your default — not your best day, but under pressure?">
                  {contrastTable.title || "Two Ways to Lead"}
                </SectionHead>
                <div className="overflow-x-auto rounded-xl border" style={{ borderColor: "#e5e7eb" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr>
                        <th style={{ background: "#f9fafb", color: "#555", fontWeight: 700, fontSize: 13, padding: "12px 16px", textAlign: "left", borderBottom: "1px solid #e5e7eb" }}>
                          {contrastTable.leftTitle}
                        </th>
                        <th style={{ background: accentLight, color: NAVY, fontWeight: 700, fontSize: 13, padding: "12px 16px", textAlign: "left", borderBottom: "1px solid #e5e7eb" }}>
                          {contrastTable.rightTitle}
                        </th>
                      </tr>
                    </thead>
                    <tbody>
            ...
            <SectionHead sub="These principles anchor everything about this dimension.">
              {principles.length} Governing Principles
            </SectionHead>
            {principles.map(p => (
              <div key={p.num} className="p-5 sm:p-6 rounded-xl" style={{ borderLeft: `4px solid ${accent}`, background: accentLight }}>
                ...
                {p.scripture && (
                  <p className="text-sm italic mb-4 pl-11" style={{ color: NAVY, borderLeft: `2px solid ${accent}`, paddingLeft: 12, marginLeft: 44 }}>
                    {p.scripture}
                  </p>
                )}
                <div className="space-y-3 mb-4">
                  {p.paragraphs.map((para, i) => (
                    <p key={i} className="text-sm leading-relaxed" style={{ color: "#333" }}>{para}</p>
                  ))}
                </div>
                {p.prompt && (
                  <div className="pt-4" style={{ borderTop: `1px solid ${accent}44` }}>
                    <p className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: NAVY }}>Pause & Process</p>
      ...
      case "exemplar":
        return (
          <div className="space-y-6">
            ...
            {exemplar.intro2 && (
              <p className="text-sm leading-relaxed" style={{ color: "#333" }}>{exemplar.intro2}</p>
            )}
      ...
      case "stages":
        return (
          <div className="space-y-5">
            <SectionHead sub="These stages are recognizable across every leader's journey. You are in one right now.">
              Stages of {title} Development
            </SectionHead>
            {stages.map((s, i) => (
              <div key={i} className="p-5 rounded-xl" style={{ background: "#fff", border: `1px solid #e5e7eb`, borderLeft: `4px solid ${accent}` }}>
                <h4 className="font-bold mb-2" style={{ color: NAVY, fontFamily: "'Cormorant Garamond', serif", fontSize: 17 }}>
                  {s.title}
                </h4>
                ...
                {s.markers && (
                  <ul className="mt-3 space-y-1.5">
                    {s.markers.map((m, j) => (
                      <li key={j} className="flex gap-2 text-xs" style={{ color: "#666" }}>
      ...
      case "post-diagnostic":
        return (
          <DiagnosticSection
            diagnostic={diagnostic}
            scores={postScores}
            setScores={setPostScores}
            accent={accent}
            label="Post-Teaching Self-Assessment — What Has Shifted?"
          />
        );
      ...
      case "commitment":
        return (
          <div className="space-y-6">
            <SectionHead sub="Revelation must become responsibility. Write in full sentences — not fragments.">
              My {title} Commitment
            </SectionHead>
            <div className="space-y-5">
            ...
            {revisitTriggers && revisitTriggers.length > 0 && (
              <div className="p-5 rounded-xl" style={{ background: accentLight }}>
                <h4 className="font-bold mb-3" style={{ color: NAVY, fontFamily: "'Cormorant Garamond', serif" }}>
                  {title} is a Living Discipline
                </h4>
                <p className="text-sm mb-3 leading-relaxed" style={{ color: "#333" }}>
                  What you write today is not a one-time exercise. Return to it regularly as your season deepens.
                </p>
      ...
      case "summary":
        return (
          <div className="space-y-6">
            <SectionHead sub={`Based on your reflections and commitments — your personalized ${title} analysis.`}>
              Your {title} Blueprint
            </SectionHead>
            {!aiSummary && !loading && (
            ...
            {loading && (
              <div className="text-center py-12">
                <div className="w-10 h-10 border-4 rounded-full animate-spin mx-auto mb-4"
                  style={{ borderColor: accentLight, borderTopColor: accent }} />
                ...
                <div className="p-6 rounded-2xl" style={{ border: `2px solid ${accent}`, background: accentLight }}>
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0"
                      style={{ background: NAVY, color: accentMid, fontSize: 14 }}>✦</div>
                    <p className="font-bold" style={{ color: NAVY, fontFamily: "'Cormorant Garamond', serif", fontSize: 17 }}>
                      Your {title} Blueprint
                    </p>
                  </div>
                  <div>
                    {aiSummary.split("\n\n").map((para, i) => (
                      <p key={i} className="text-sm leading-relaxed mb-3" style={{ color: "#222" }}>{para}</p>
                    ))}
                  </div>
                </div>
                <button onClick={() => downloadBlueprint(title, commitments, aiSummary)}
                  className="w-full py-3 rounded-2xl font-semibold text-sm transition-all"
                  style={{ border: `2px solid ${NAVY}`, color: NAVY, background: "#fff" }}>
    ...
    <div className="min-h-screen" style={{ background: "#FAFAF8", fontFamily: "'Outfit', sans-serif" }}>
      ...
      <div className="sticky top-0 z-50 border-b" style={{ background: "rgba(255,255,255,0.92)", backdropFilter: "blur(12px)", borderColor: "#f0f0f0" }}>
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
          <a href="/" className="text-sm font-medium flex items-center gap-1.5 hover:opacity-70 transition-opacity" style={{ color: NAVY }}>
            ← Dashboard
          </a>
          <div className="text-center">
            <p className="text-xs uppercase tracking-widest font-semibold" style={{ color: NAVY }}>
              {moduleNum === 0 ? "Introduction" : moduleNum === 6 ? "Commissioning" : `Module ${moduleNum}`}
            </p>
        ...
        <div className="flex items-center gap-1 overflow-x-auto pb-1 scrollbar-hide">
          {STEPS.map((s, i) => (
            <button key={s.id} onClick={() => { setStep(i); scrollTop(); }}
              ...
              style={{
                background: i === step ? accent : i < step ? accentLight : "transparent",
                color: i === step ? NAVY : i < step ? accent : "#bbb",
      ...
      <div className="max-w-3xl mx-auto px-4 pb-36" ref={topRef}>
        <div className="mb-8">
          ...
          <h2 className="text-2xl sm:text-3xl font-bold" style={{ color: NAVY, fontFamily: "'Cormorant Garamond', serif" }}>
            {title}{subtitle ? `: ${subtitle}` : ""}
          </h2>
          {question && (
            <p className="text-sm mt-1 italic" style={{ color: "#888" }}>
      ...
      {/* BOTTOM NAV */}
      <div className="fixed bottom-0 left-0 right-0 border-t py-3 z-40"
        style={{ background: "rgba(255,255,255,0.95)", backdropFilter: "blur(12px)", borderColor: "#f0f0f0" }}>
        <div className="max-w-3xl mx-auto px-4 flex justify-between items-center">
          <button
            onClick={() => { if (step > 0) { setStep(step - 1); scrollTop(); } }}
            disabled={step === 0}
          ...
          <button
            onClick={() => { if (step < STEPS.length - 1) { setStep(step + 1); scrollTop(); } }}
