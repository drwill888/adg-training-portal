...
const COLORS = { navy: '021A35', gold: 'C8A951' };
...
export default async function handler(req, res) {
  ...
  try {
    ...
    children.push(
      ...
      new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 400, after: 400 }, children: [new TextRun({ text: 'AWAKENING DESTINY GLOBAL', font: FONTS.ui, size: 18, color: COLORS.gold, characterSpacing: 40 })] }),
      new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: generatedDate, font: FONTS.ui, size: 18, color: '888888' })] }),
    ...
    for (const block of blocks) {
      if (block.startsWith('## ')) {
        ...
        children.push(new Paragraph({ spacing: { before: 120, after: 180, line: 360 }, children: runs }));
      }
    }
    ...
    const doc = new Document({
      ...
      title: `Final 5C Leadership Blueprint — ${firstName}`,
      description: 'Complete prophetic-strategic Kingdom leadership formation report',
    ...
    const filename = `5c-final-blueprint-${firstName.toLowerCase()}.docx`;
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    res.setHeader('Content-Length', buffer.length);
    return res.status(200).send(buffer);
  } catch (err) {
    console.error('Final blueprint download error:', err);
